...
export function SlaCards({ scorecards }: { scorecards: ReliabilityScorecard[] }) {
  if (scorecards.length === 0) {
    return (
      <div className="panel p-6 text-sm text-tower-muted" role="status">
        No sources registered yet. Add a demo connector to see SLA scorecards.
      </div>
    );
  }
  ...
  return (
    <div className="grid gap-4 md:grid-cols-3">
      {scorecards.map((card) => {
        const freshOk = card.freshness_status === "ok";
        return (
          <article key={card.source_id} className="panel p-4">
            <div className="mb-3 flex items-start justify-between gap-3">
              ...
              <span
                className={`mt-1 inline-flex items-center gap-2 text-xs uppercase tracking-wide ${
                  freshOk ? "text-tower-accent" : "text-tower-danger"
                }`}
              >
                <span
                  className={`inline-block h-2.5 w-2.5 rounded-full ${statusDot(freshOk ? "ok" : "fail")}`}
                  aria-hidden
                />
                <span>{freshOk ? "within SLA" : "breach"}</span>
              </span>
            </div>
            ...
            <div
              ...
              role="img"
              aria-label={`Reliability trend: ${card.trend.join(", ")}`}
            >
